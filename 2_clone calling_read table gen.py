...
maxMM = int(input('enter max MM for removing bc-shadows'))
input_3_out2_Masterdict = open('out_2_'+sample_name+'_MD.sam','r')
output_3_clean = open('out_3_'+sample_name+ 'bc' + str(maxMM)+'MM_MD_clean.txt','w')
output_4_class = open('out_4_'+sample_name+ 'class.txt','w')
output_3_kicked = open('out_3_'+sample_name+ 'bc' + str(maxMM)+'MM_kicked_shadows.txt','w')
...
for line in input_3_out2_Masterdict:
    ...
    if gDNAname_n+exp_name_n == gDNAname+exp_name: #same guide same exp
        ...
        for bc_c_tuple in barcode_list:        #now check reference list
            ...
            if count_n <= 1 or (MMmax(barcode_n,ref_barcode,maxMM) and count_n * 3 <= ref_count):
                shadow_found = 'yes'
...
output_4_class = open('out_4_'+sample_name+ 'class.txt','r')
output_5_class = open('output_5_'+sample_name+ 'class.txt','w')
...
output_5_table = open('output_5_' + sample_name + '.tsv','w')
#write header
